...
public class PlaylistDatabase {
    ...
    public boolean createAndAddTrackToPlaylist(Long guildId, String name, List<AudioTrack> tracks, Long ownerId, PlaylistRights rights) {
        
        if (!createPlaylist(guildId, name, ownerId, rights)) {
            return false;
        }

        for (AudioTrack track : tracks) {
            if (!addTrackToPlaylist(guildId, name, track.getInfo().title, track.getInfo().uri)) {
                return false;
            }
        }
